...
import { useGetEntryByIdQuery, usePostDeleteEntryMutation, usePostUpdateEntryMutation, useSetting} from '../../../../state/entry';
...
const EditEntry = props => {
  ...
  const updateEntryFuc = () => {
    updateEntry({entryText,visiblity:data?.journalEntry?.privacy,hashtags: myhashtags,
    comment,journal:dropdownValue?._id,imageArray: imageIdArray,
    status:"published"
    }).then(res => {
      if (res?.data) {
